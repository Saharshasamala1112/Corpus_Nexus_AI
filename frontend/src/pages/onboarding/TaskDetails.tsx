...
function TaskDetails() {
  ...
  const task = useMemo(() => onboardingTasks.find((t) => t.id === Number(id)), [id])

  const currentTask = task
  const progress = getProgress()
  const completedTask = currentTask ? progress[currentTask.id] : undefined

  const [previewUrls, setPreviewUrls] = useState<string[]>(completedTask?.images || [])
  ...
  return (
    <div className="task-details-page">
      ...
      <div className="upload-section">
        ...
        <p className="hint">Upload up to 5 screenshots as proof of completion.</p>
