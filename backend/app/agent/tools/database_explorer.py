...
class DatabaseExplorerTool(BaseTool):
    ...
    def definition(self) -> ToolDefinition:
        return ToolDefinition(
            ...
            keywords=[
                "database", "db", "schema", "table", "column", "field",
                "postgresql", "postgres", "sql", "model", "migration",
                "relationship", "foreign key", "index", "primary key",
                "orm", "sqlalchemy", "django", "entity",
            ],
        )
    ...
    def matches_query(self, query: str) -> float:
        ...
        db_terms = ["database", "schema", "table", "column", "model", "migration",
                     "relationship", "foreign key", "index", "sql", "orm", "postgresql"]
    ...
    async def execute(self, parameters: dict) -> dict:
        ...
        for r in results:
            ...
            if "model" in file_path.lower() or "migration" in file_path.lower() or ".py" in file_path:
        ...
        logger.info(
            ...
            query[:50], len(table_list), elapsed_ms,
        )
...
def _extract_type(col_def: str) -> str:
    type_match = re.search(r"(String|Integer|Float|Boolean|Text|DateTime|Date|UUID|JSON|ForeignKey|LargeBinary|BigInteger|Numeric)", col_def, re.IGNORECASE)
    if type_match:
