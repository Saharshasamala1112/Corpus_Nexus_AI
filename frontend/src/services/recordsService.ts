...
export const getRecords = async (skip = 0, limit = 1000): Promise<CorpusRecord[]> => {
  try {
    const response = await api.get<CorpusRecord[]>(`/records?skip=${skip}&limit=${limit}`)
...
export const getRecordsPaginated = async (pages = 3): Promise<CorpusRecord[]> => {
