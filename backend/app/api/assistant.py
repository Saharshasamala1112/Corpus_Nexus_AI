...
async def chat_endpoint(request: Request, payload: dict):
    ...
    await message_repo.create(f"msg-{int(time() * 1000)}-user", conversation_id, "user", question)
    await message_repo.create(f"msg-{int(time() * 1000)}-assistant", conversation_id, "assistant", answer)
...
async def chat_stream_endpoint(request: Request):
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            await message_repo.create(assistant_id, conversation_id, "assistant", accumulated)
            return
...
async def create_message(conv_id: str, request: Request, payload: dict):
    ...
    if not conv:
        await conversation_repo.create(conv_id, payload.get('title') or 'Conversation', model=None)
...
async def upload_file(request: Request, file: UploadFile = File(...)):
    ...
    return JSONResponse(content={"url": f"/uploads/{os.path.basename(path)}", "filename": file.filename})
