...
function buildSpecificMediaCountAnswer(
  lower: string,
  counts: CountMap,
): InsightResponse {
  for (const mt of MEDIA_TYPES) {
...
function isContributorCountQuery(lower: string): boolean {
  return (
    (lower.includes("number") || lower.includes("count")) &&
    lower.includes("contributor")
  );
}
...
async function buildLeaderboardAnswer(
  lower: string,
