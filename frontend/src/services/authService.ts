...
export async function getInstitution(id: string): Promise<InstitutionInfo> {
...
export async function getUserContributions(userIdentifier: string): Promise<ContributionData> {
  const response = await api.get(`/users/${userIdentifier}/contributions`)
  return response.data
}
